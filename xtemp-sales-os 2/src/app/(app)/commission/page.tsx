...
import { getWonDealsForYear } from "@/lib/data";
...
export default async function CommissionPage({
  ...
  const wonDeals = await getWonDealsForYear(supabase, year);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <ButtonLink href="/commission/tiers" variant="secondary" size="sm">
            Rate table
          </ButtonLink>
        </div>
      </div>
            ...
            {monthly.map((m) => (
              <tr key={m.month} className="border-b border-line last:border-b-0 hover:bg-paper/60">
                ...
                <td className="px-5 py-3 text-right">
                  {m.count > 0 && (
                    <Link href={`/commission/${year}/${m.month}`} className="text-xs text-wire hover:underline">
                      View report →
                    </Link>
                  )}
                </td>
