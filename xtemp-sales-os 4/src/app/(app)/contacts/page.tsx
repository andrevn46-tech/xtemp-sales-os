...
export default async function ContactsPage({
  ...
  const contacts = await getAllContacts(supabase, { status: params.status, search: params.q });
    ...
    <div className="flex flex-col gap-6">
      ...
      <form method="get" className="flex flex-wrap gap-3">
        ...
        {(params.status || params.q) && (
          <Link href="/contacts" className="text-sm px-4 py-2 text-ink-dim hover:text-ink">
            Clear
          </Link>
        )}
      </form>
